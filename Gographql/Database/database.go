...
var connectionString = "mongodb+srv://chandelsanjeev5:saksham123@fiber.oksyndw.mongodb.net/?retryWrites=true&w=majority"
...
func (db *DB) GetJob(id string) *model.JobListing {
	jobCollec := db.client.Database("Fiber").Collection("jobs")
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
...
func (db *DB) CreateJobListing(jobInfo model.CreateJobListingInput) *model.JobListing {
	...
	inserg, err := jobCollec.InsertOne(ctx, bson.M{
		"title":       jobInfo.Title,
...
func (db *DB) UpdateJobListing(jobId string, jobInfo model.UpdateJobListingInput) *model.JobListing {
	...
	if jobInfo.Description != nil {
		updateJobInfo["description"] = jobInfo.URL
	}
	...
	if err := results.Decode(&jobListings); err != nil {
		log.Fatal(err)
	}
...
func (db *DB) DeletJobListing(jobId string) *model.DeleteJobResponse {
	jobCollec := db.client.Database("Fiber").Collection("jobs")
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
